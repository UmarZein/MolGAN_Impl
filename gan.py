...
class GraphGANModel(nn.Module):
    # terminology note: decoder_hidden_dims is basically decoder_hidden_dim
    def __init__(self, vertexes, edges, nodes, embedding_dim, decoder_hidden_dims, discriminator_hidden_dims,
                 decoder, discriminator, soft_gumbel_softmax=False, hard_gumbel_softmax=False,
                 batch_discriminator=True):
        self.vertexes, self.edges, self.nodes, self.embedding_dim, self.decoder_hidden_dims, self.discriminator_hidden_dims, \
        self.decoder, self.discriminator, self.batch_discriminator = vertexes, edges, nodes, embedding_dim, decoder_hidden_dims, \
                                                                     discriminator_hidden_dims, decoder, discriminator, batch_discriminator

        self.training = False
        self.dropout_rate = 0
        self.soft_gumbel_softmax = soft_gumbel_softmax
        self.hard_gumbel_softmax = hard_gumbel_softmax
        self.temperature = 1

        self.edges_labels = nn.Parameter(torch.none(None, vertexes, vertexes))
        self.nodes_labels = nn.Parameter(torch.none(None, vertexes))
        ...
        with tf.variable_scope('generator'):
            self.edges_logits, self.nodes_logits = self.decoder(self.embeddings, decoder_hidden_dims, vertexes, edges, nodes,
        ...
        with tf.name_scope('D_x_real'):
            self.logits_real, self.features_real = self.D_x((self.adjacency_tensor, None, self.node_tensor),
                                                            units=discriminator_hidden_dims)
        ...
        with tf.name_scope('V_x_fake'):
            self.value_logits_fake = self.V_x((self.edges_hat, None, self.nodes_hat), units=discriminator_hidden_dims)
    ...
    def sample_z(self, batch_dim):
        return torch.randn(batch_dim, self.embedding_dim)